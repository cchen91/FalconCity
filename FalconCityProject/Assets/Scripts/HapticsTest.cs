...
public class HapticsTest : MonoBehaviour
{
    ...
    public static HapticsTest main;
    #endregion
    ...
    {
        if (c.collider.tag == "Sand" || c.collider.tag == "Water" || c.collider.tag == "Surface")
        {
    ...
    private void OnCollisionStay(Collision c)
    {
        ...
        if (c.collider.tag == "sand")
        {
            SpeedX = -viscocity * (origContactObjectPoint.x - vectorProj.x);
            SpeedY = -viscocity * (origContactObjectPoint.y - vectorProj.y);
            SpeedZ = -viscocity * (origContactObjectPoint.z - vectorProj.z);
        }
    }

    private void OnCollisionExit(Collision c)
    {
        if (c.collider.tag == "Surface" || c.collider.tag == "water" || c.collider.tag == "sand")
        {
            //Reset everything
            PosX = 0.0f;
            PosY = 0.0f;
    ...
    private void _feedback()
    {
		...
		if (flag == 0)  SetServoPos(new double[3] { PosX, PosY, PosZ }, Strength);
        if (flag == 1)  SetServo(new double[3] { SpeedX, SpeedY, SpeedZ});
        if (flag == 2)
        {
